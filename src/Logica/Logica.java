...
import Datos.Candidatos;
import Datos.Electores;
...
public class Logica {
            ...
            do{
                ...
                for (int j = 0; j < electores.size(); j++) {
                    if( electores.get(j).cedula.compareTo( ob.cedula )==0 ){ //el compareto compara dos string
                        encontrado = 1;
                        System.out.println("**** la cedula del elector ya fue ingresada ****");
                    }
                }
    ...
    public void sumarVotosObtenidosPorCandidato(){
        ...
        for (int i = 0; i < electores.size(); i++) {
            int temporal = (int) candidatos.get(electores.get(i).opcion).votosObtenidos+1; //guarda en temporal un voto mas para el candidato de la posicion del get
